from app import db
from models import Transaction, TransactionType, CreditSource, CreditPayment
...
def create_new_transaction(amount_in_cents:int, type:TransactionType, description:str):
    ...
    try:
        ...
        session.commit()
        print('Successfully created transaction of type ' + str(type) + ' and amount ' + str(amount_in_cents) + '\n' + description)
    except:
...
def create_new_credit_payment(amount_in_cents:int, type:CreditSource, description):
    new_payment = CreditPayment(amount_in_cents=amount_in_cents, credit_type=type, description=description)
    try:
        session.add(new_payment)
        session.commit()
        print('Successfully create credit payment for ' + str(type) + ' and amount ' + str(amount_in_cents) + '\n' + description)
    except:
