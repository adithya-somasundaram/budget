...
TRANSACTION_TYPE_ROWS = [(1, "Credit"), (2, "Debit"), (3, "Cash"), (4, "Check"), (5, "Venmo")]
...
def new_table(*columns, justify_first_right=True) -> Table:
    ...
    for i, column in enumerate(columns):
        justify = "right" if (i == 0 and justify_first_right) or column in ("Balance", "Amount", "Remaining") else None
...
def account_value_table(accounts, title="Account", show_total=False) -> Table:
    ...
    if show_total:
        table.add_section()
        table.add_row("", "[bold]TOTAL[/bold]", f"[bold]{cents_to_dollars_str(grand_total)}[/bold]")
    return table
